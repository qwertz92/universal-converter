...
>
> **Status note:** as of this writing, `data/sources.json` contains the three
> seed entries used by the shipped unit catalog (`data/units.json`). The
> fuel/emission sources described below (DESNZ, IPCC, EPA, EIA, H2Tools,
> SEAI, EEA) are documented here from `docs/research-notes.md` ahead of the
> Data agent populating `data/fuels.json` / `data/emission-factors.json` and
> adding the corresponding entries to `data/sources.json`. Treat this file as
> the target source register for the full v0.1 fuel/emissions catalog, not
> only the units already shipped.
...
| **US EPA GHG Emission Factors Hub 2025** (`epa-ghg-hub-2025`) | US EPA (Center for Corporate Climate Leadership) | US federal government work — public domain | **US-specific cross-check**: Scope 1/2/3 factors, eGRID electricity intensity by subregion, used to sanity-check DESNZ/IPCC order of magnitude and to offer an explicit US alternative, never blended with UK/global figures | High — 435 factors, widely used for corporate GHG reporting; traces to 40 CFR Part 98 / the US GHG Inventory. [epa.gov](https://www.epa.gov/climateleadership/ghg-emission-factors-hub) |
...
| **H2Tools (Hydrogen Tools)** (`h2tools`) | Pacific Northwest National Laboratory for US DOE Hydrogen Safety Panel | US DOE-funded public resource; treat as citable with attribution | Hydrogen physical properties: LHV 119.96 MJ/kg (33.3 kWh/kg), HHV 141.88 MJ/kg (39.4 kWh/kg), gas/liquid density | High by reputation and cross-source consistency; our own confidence is **medium** because the primary page blocked automated fetch (HTTP 403) during research — figures are cross-verified via independent secondary sources citing DOE/NIST data. Flagged for a manual re-check before being treated as fully first-hand. |
| **SEAI Conversion & Emission Factors** (`seai-conversion-factors`) | Sustainable Energy Authority of Ireland | Irish public-sector data; exact license terms not yet confirmed | Potential EU-country cross-check (not yet mined into the data files) | Medium-high; verify the license page before bulk reuse |
...
| **Engineering ToolBox** (`engineering-toolbox`) | Private commercial site | Not a licensed dataset | **Plausibility cross-check only** — never a primary citation for any shipped value | Low-medium as a primary source; fine for sanity-checking order of magnitude, per project rules (spec §6.1/§12) |
| **UCUM**, **QUDT** | Regenstrief Institute / QUDT.org | Open specifications | Reference vocabularies for unit ids/aliases/ontology design — not numeric-factor sources | N/A — design references only |
| **OPEC Annual Statistical Bulletin** | OPEC | Publicly downloadable PDF; license not fully assessed | Potential source for an OPEC-specific `boe` convention note | Not fully verified — flagged as a research gap; not currently cited for any shipped value |
