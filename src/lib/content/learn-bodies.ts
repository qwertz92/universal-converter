...
<p>The therm rounds off to a tidy hundred-thousand BTU precisely because it was designed as a billing unit. That tidiness is a matter of convention, not physics: there is nothing in nature that prefers 100,000 BTU. The tool labels the therm as a defined convention. Both the US and the UK/EC therm are defined through the same chain — 100,000 British thermal units on the International Table definition — so this catalog claims no divergence between them; an earlier version of this page asserted one at the 10^-5 level, which nothing in the source ledger supports.</p>
...
<ul>
<li>For natural gas, HHV exceeds LHV by roughly <code>~11%</code> — the catalog's own figures give 45.745 → 50.678 MJ/kg. Methane is the same. The often-quoted <code>~5–6%</code> is the figure for petrol and coal, not for gas.</li>
